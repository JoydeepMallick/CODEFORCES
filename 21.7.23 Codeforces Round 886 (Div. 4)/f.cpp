...
void test(){
    ...
    unordered_map<ll,ll> nooffrogs; // unique jumps 
    for(auto frogjump : a){
        if(frogjump <= n) nooffrogs[frogjump]++;//diregard frogs who jump more than n at once since they can be caught ever
    }
dbg(nooffrogs);


    unordered_map<ll,ll> jumps;
    for(auto it : nooffrogs){
        //check path of each frog
        ll start = it.fi; // jump
        while(start <= n){
            jumps[start] += it.se;
            start += it.fi;
...
/* dbg(jumps); */
    ll ans = 0;
    for(auto jmpcnt : jumps){
        ans = max(ans, jmpcnt.se);
    }
