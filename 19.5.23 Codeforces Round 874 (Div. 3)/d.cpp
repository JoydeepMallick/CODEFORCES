...
void test(){
    ...
    map<int, int> freq;
    fori(0,n){
        if(freq.find(a[i]) == freq.end()) uniquea.push_back(a[i]); 
        freq[a[i]]++;
    }
    sort(uniquea.begin(), uniquea.end());
    
    vector<ll> diff(uniquea.size()-1);

    fori(0,uniquea.size()-1){
        diff[i] = uniquea[i+1]-uniquea[i];
    }
    ...
    dbg(uniquea, diff, freq);
    while(r < uniquea.size()){
        ll temp = 1;
        if(s < m){
            fori(l, r+1){
                temp = (temp * freq[uniquea[i]]) % mod; 
            }
        }
        ans = (ans + temp) % mod;
        s = s - uniquea[l] + uniquea[r];
        l++;
        r++;
    }
