...
void test(){
    ...
    fori(0,2*n-2){
        string s;
        cin >> s;
        // we have all possible prefixed except for length n , so comparing the string s of length (n-1) will tell us the answer.
        if(s.size() == (n-1)) presuf[ind++] = s;
    }
    // if string is "zkikz" then "zkik"  and "kikz" so reverse any one and check if they are equal
    reverse(presuf[1].begin(), presuf[1].end());
    dbg(presuf[0], presuf[1]);
    if(presuf[0] == presuf[1] ) yes;
    else no;
