...
void test(){
  ll n;
  cin >> n;
// when points are on axis lets count them for both relevant quadrants else count the number of points in each quadrant, only points lying in 2 adjacents quadrants are allowed.
  vll quad(5,0);
  int yp = 0, yn = 0, xp = 0, xn = 0;
  fori(0,n){
    int x,y;
    cin >> x >> y;
    ...
    dbg(quad);
//alternate quad not allowed at ant cost 
    if( (quad[1]|quad[3]) > 0 || (quad[2]|quad[4]) > 0) no;
    else if()
    else yes;
