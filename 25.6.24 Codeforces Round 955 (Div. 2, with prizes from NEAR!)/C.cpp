...
int f(vll &a, ll l, ll r, ll ind, int s, vll &dp) {
  if (ind == a.size()) {
    return 0;
  }
  if (ind == 0) {
  ...
  s += a[ind];
  if (s >= l && s <= r) {
    return dp[ind] = 1 + f(a, l, r, ind + 1, 0, dp);
  } else {
    // add next
    ll addnext = f(a, l, r, ind + 1, s, dp);
    // skip
    ll skip = f(a, l, r, ind + 1, 0, dp);
    return dp[ind] = max(addnext, skip);
  }
}
