            ...
            dbg((1 << i) & v);
              if( ((1 << i) & v) != 0){
                  if(pow2[i] > 0) pow2[i]--;
                      ...
                      //if possible with lower powers of 2 provided they exist in sufficient amount
                      possible = false;
                      rfor(j, i-1, -1){
                          if(pow2[j] > 1){
                              //check highest power of 2 possible and if greater then yes 
                              ll highestpossiblepower = j + floorl(log2(pow2[j]));
                              dbg(highestpossiblepower);
                              if(i <= highestpossiblepower){
                                  pow2[j] -= (i-j);
                                  possible = true;
                                  break;
                              }
