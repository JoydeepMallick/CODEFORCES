...
void test(){
    ull n;//dont forget to keep n large enough
    cin >> n;
    //max possible kills only possible if we remians arround sqaures as much as possible
    //series of square sum + 1.2+ 2.3+ ...+ n-1.n 
    ull ans  = ((((n*(n+1))%mod)*(4*n-1))%mod*337)%mod;
/* ull ans = ( ( ( ((n*(n+1)) %mod) *(4*n-1) ) %mod) * (2022/6) ) % mod;//mod in 3 layers */
    cout <<  ans << endl;
