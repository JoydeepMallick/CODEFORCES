...
void test(){
  int n,m,valid = 1;cin >> n >> m;
  string a,b;
  cin >> a >> b;
  int j = n-1;
  //check if at least till 2nd element of b is same as till last m-1 elements of a
  for(int i = m-1;i > -1; i--){
    if(a[j] != b[i] && i > 0) {
      valid = 0;
      break;
  ...
  if(valid && j){//if all elements already same no need to check
    valid = 0;
    fori(0,j) if(a[i] == b[0]){
    valid = 1;
    break;
