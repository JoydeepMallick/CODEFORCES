...
void test(){
    ...
    //we must minimize f and under all satisfactory condtions lin value can be 0 or greater.
    //
    //now xi and yi must be greater than or equal to s.
    //
    //again xi + yi is ai 
    //
    //so our target is x2, y2 and xn-2, yn-1 others y3 to yn-2 and similar for x will be in a format such that one value is s and other is ai-s
    //
    //we need to manipulate x2 and yn-1 that they are minimum.


    ull x2 = s;//min value of possible
    ull yn_1 = s;//min value possible
    //given condition suggest s <= xi and yi and ai definitely
    ull prod = a[1]*x2 + yn_1*a[n];
    fori(2,n){
        //compute the rest of product
        if(i == n-1){
            prod +=  (a[i]-s)*(a[n]-s);
        }
        else {
