...
void test(){
  ...
  //from observation we can definitely say that sorting till the minimum elements
  //since when we take the minimum element every move causes no change to array 
  //so if the elements after minimum elements are in sorted fashion then its ok and no of steps is equal to  no of elements before the minimum element.
  //

  int minele = a[0], indofmin = 0;
  fori(1,n){
    if(minele > a[i]) indofmin = i, minele = a[i];
  }
