...
//assume temp array as   2  5  4  1 -2 -5 6 -9 4 -8 5 2  

//sum then becomes       2  7 11 12 10 5 11  2 6  -8 5 7      maxsum = 12 
//                                                |
//                                          restarting sum since we are interested max sum and negative sum in no order will benefit us, add a negative number though would be benefiical if in a long run inserting  a negative might result  in increasing length of array and open doors to large positives waiting away from it. 
//
//e.g.  3 4 -5 -2 9 
// sum  3 7  2 -2 9   so maxsum is 9 and not 7 since when we insert two negatives we open door for substantial large positive
//      here we take -2 into consideration next time on addition but get rejected in next iteration 
//
//      for eg.  -100 -333 -199 
//                0    0     0 
//                -100  -333   -199       maxsum = -100
//                                    
ll max_subarr_sum(vll temp){
  ll maxsum = LONG_MIN, sum = 0;
  for(auto ele : temp){
    sum = max(sum, 0ll);
    sum += ele;
...
void test(){
  ...
  //first find all dividers position i.e. numbers with same parity
  vll dividers;// each value is like till that index but not that index itself
  dividers.pb(0);
  fori(0, n-1){
    //a[i]%2 == a[i+1]% 2 does not work since in c++ it returns -1 and 1 in case of -ve and positive number strangely
    dbg(a[i]%2, a[i+1]%2);
    if((a[i] + a[i+1])%2 == 0) dividers.pb(i+1);
  }
  dividers.pb(n);
  dbg(dividers);

  //find max sum between all the dividers.
  //  0 fixed left boundary|.........|.........|.......|......|n fixed right boundary
  //  maxsum   maxsum    maxsum maxsum
  //
  //  the max value of all maxsums of all subarrays will be the answer

  //we know for sure the loop runs once for minimum since dividers size >= 2
  fori(0, dividers.size()-1){
    vll temparr = vll(a.begin() + dividers[i],  a.begin() + dividers[i+1]);
    ll subsum = max_subarr_sum(temparr);
    dbg(temparr, subsum);
