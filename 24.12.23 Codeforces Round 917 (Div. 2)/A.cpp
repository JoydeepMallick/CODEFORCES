...
void test(){
  ...
  /* at max 0 or 1 operation needed */
  /* if all even number(0,2,4,....) of negative elements then one negative element must be made 0 */
  /* else if only one element and positive then replace it to 0 else  no operations needed */
  ll poselecnt = 0, negelecnt  = 0;

  fori(0,n){
    if(a[i] < 0) negelecnt++;
    else if(a[i] > 0) poselecnt++;
  }

  if((n - negelecnt - poselecnt) > 0){
    //0 exists
    cout << "0\n";
  }
  else if(negelecnt > 0){
