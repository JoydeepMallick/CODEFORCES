...
void test(){
    ...
    ll cnt = n; // we are sure x with x i.e. xx will always be counted 

    fori(0,n){
        string si = a[i];
        forj(i+1, n){
            string sj = a[j];
            if( (si.size() + sj.size()) % 2 == 0){
                int midind = (si.size() + sj.size())/2;
                //for sisj
