...
void test(){
  ...
  //apporach :- 
  /* we just need 3 indices of 3 highest values per array and then we can surely do brute forcing on indices in 3^3 time when indices are not equal and then the sum of values of those indices is surely maximum. */
  /* Now assume 1st max or 2nd max of a particular array same or 3rd max and 4th max is same then no matter what is the index we are just concerned with the sum and surely the sum remains the same. */
  ll maxsum = 0;
  vll maxindicesofa = max3indices(a), maxindicesofb = max3indices(b), maxindicesofc = max3indices(c);
  for(auto x : maxindicesofa){
    for(auto y : maxindicesofb){
      for(auto z : maxindicesofc){
        if(x != y && y!= z && x!=z){
          maxsum = max(maxsum, a[x] + b[y] + c[z]);
        }
      }
    }
