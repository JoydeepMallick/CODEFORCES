...
void test(){
  ...
  //greedy apporach would be to go till certain i quest once and then onwards the remaining to be done from the quests from 0 to i which gives max value.
  //
  //check all such possibilities on a go for each i and max is the answer.
  ll maxbtilli = 0;
  ll ans = 0;
  ...
  fori(0, n){
    if(k == 0) break;
    maxbtilli = max(maxbtilli, b[i]);
    sumatilli += a[i];
    k--;
    ans = max(ans, sumatilli + k*maxbtilli);
  }
