...
void test(){
    ...
    //when ever monster health decreases to similar value like others we can cast a spell to reduce all their health by one but we will do it only when freq of such monster is more than the healthpoint else kill each of them is better
    //but constaint is  to reduce of only two
    //hence first try to kill all unique health monsters
    mii freq;
    fori(0,n){
        freq[h[i]]++;
    }
    ...
    for(auto it : freq){
        if(it.second >= 2){
            //reduce health or kill all
            moves += min(it.first * it.second/2 + it.second%2, it.second);
/* dbg(moves); */
        }
        else moves++;
    }
