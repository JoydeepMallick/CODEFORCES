...
void test() {
  ...
  vector<pair<int, int>> slices; // 000..1111... slices
  slices.pb({0, 0});
  fori(1, s.size()) {
    if (s[i] >= s[i - 1]) {
      slices[slices.size() - 1].second++;
      continue;
    } else {
      slices.pb({i, i});
    }
  ...
  int max01slicesize = 0;
  for (auto sl : slices) {
    if (s[sl.first] != s[sl.second]) {
      // a 01 slice
      max01slicesize = max(max01slicesize, (sl.second - sl.first + 1));
    }
  }
  ...
  // remove the slice
  fori(0, slices.size()) {
    pair<int, int> sl = slices[i];
    if (s[sl.first] != s[sl.second] &&
        (sl.second - sl.first + 1) == max01slicesize) {
      todelind = i;
      break;
    }
  }
  if (todelind != -1)
    slices.erase(slices.begin() + todelind);
  dbg(slices);
  for (auto rem : slices) {
    if (s[rem.first] != s[rem.second]) {
      cnt += 2;
    } else {
      cnt += 1;
    }
