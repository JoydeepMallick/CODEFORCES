...
void test(){
  ...
  map<vector<ll>, int> coord;


  int x[] = {0,0,1,1};
  int y[] = {0,1,0,1};
  fori(0,4){
    vll c1(2),c2(2), c3(2), c4(2);
    c1[0] = xk + powl(-1, x[i]) * a;
    c1[1] = yk + powl(-1, y[i]) * b;
    
    coord[c1]++;

    c2[0] = xq + powl(-1, x[i]) * a;
    c2[1] = yq + powl(-1, y[i]) * b;

    coord[c2]++;

    c3[0] = xk + powl(-1, x[i]) * b;
    c3[1] = yk + powl(-1, y[i]) * a;
    
    coord[c3]++;

    c4[0] = xq + powl(-1, x[i]) * b;
    c4[1] = yq + powl(-1, y[i]) * a;

    coord[c4]++;
  }
  //assume all poosible posittions and check coordinates with frequency > 2  
