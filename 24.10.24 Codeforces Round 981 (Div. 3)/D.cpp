...
void test(){
    ...
    vector<vector<ll>> range;
    
    mp[0].pb(0);
    fori(1, n+1){
      pref[i] = pref[i-1] + a[i-1];
      mp[pref[i]].pb(i);
      ll msize = mp[pref[i]].size();
      if(msize > 1){
        vll r = {mp[pref[i]][msize-2], mp[pref[i]][msize-1]};
        range.pb(r);
      }
    }
...
*/
    sort(all(range), bysec);
    dbg(range);
    ll ans = 0;
    //first interval always pick
    if(range.size() > 0){
      ans++;
    }
    //greedy choose next
