...
void test(){
    ...
    fori(1, n+1){
        ...
        int f, s = i, t = n;
        forj(2, k){
            //reverse walking like ........f,s,t    where f+s = t 
            f = t - s;
            t = s;
            s = f; 
            ok &= (t >= s &&  f >= 0);//f must be non negative in the edge and hence third must be >= second 
            if(!ok) break;// if it fails for certain i then no more can it be for such i hence move to next i to check
        }
        if(ok) ans++;
    }
    cout << ans << endl;
