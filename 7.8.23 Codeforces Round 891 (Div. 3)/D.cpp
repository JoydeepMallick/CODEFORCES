...
, where ci=ai−bi and our inequality is transformed to cu≥cv

.

Suppose the set p1,…pm
is the set of such vertices v that cv is maximum possible. From each pi there will be a path to all other vertices, because cpi is not less than any other cu, so the set p
...
, that is not maximum, there is no path to any vertex from p. The first observation, that there is no edge between v and any pi. So the path must go through other vertices. But even if there exist a path to another vertex u, cu will be still less that cpi, so it is impossible to get any pi
...
.
    ...
    fori(0,n){
        if(a[i]-b[i] > mx){
            ...
            store_maxdiff.pb(i+1);
        }
    }

    cout << store_maxdiff.size() << endl;
    print(store_maxdiff);
