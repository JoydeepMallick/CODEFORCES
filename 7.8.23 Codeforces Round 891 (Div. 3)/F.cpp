...
{ai+aj=bai⋅aj=c
the quadratic equation will be x2−bx+c=0, here ai and aj

will be its roots.

To find the roots of the quadratic equation, we can use the discriminant formula, D=b2−4ac
. The roots will then be x1=b−D√2 and x2=b+D√2
 ...
 */
    ...
    while(q--){
        ...
        else if(D == 0 || powl(ceill(sqrtl(D)), 2) != D){ // if D < 0 or non integer sqaure roots
          //same roots 
          cout << 1ll*freq[r1]* (freq[r1]-1)/2 << " ";
        }
        else{
          cout << 1ll*freq[r1]*freq[r2] << " ";
        }
    }
