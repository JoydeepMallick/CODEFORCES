...
void test(){
    ...
    //all 0s after this index, intially total size meaning no index rounded till now 
    int last_ind_rounded = ans.size();
    rfor(i, ans.size()-1, 0){// from last index till 2nd element, first element is 0 
        if(ans[i] >= '5'){
            if(ans[i-1] == '9'){
                int j = i-1;
                while(ans[j] == '9'){
                ...
                ans[i-1]++;
                last_ind_rounded = i-1;
            }
    }
    }
    dbg(ans);
    //keep first element if not 0 
    for(int i = (ans[0] == '0'); i < ans.size(); i++){
        cout << ((i > last_ind_rounded) ? '0' : ans[i]);
    }
