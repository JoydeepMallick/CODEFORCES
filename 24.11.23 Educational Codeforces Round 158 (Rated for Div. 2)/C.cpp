...
void test(){
  ...
  //if sort the array notice that the values position remains in sorted position even though its updated so its safe to consider only the max and min of the array in order to compute the answer
  ll mx = MAX(a);
  ll mn = MIN(a);
  if(n == 1) {
    cout << 0 << endl;
  ...
  while(mx != mn){
    ll x = mn; 
    if(maxsteps < n) xvalue.pb(x);
    mx = (mx + x)/2;
    maxsteps++;
  }
  cout << maxsteps << endl;
  if(maxsteps <= n) print(xvalue);
