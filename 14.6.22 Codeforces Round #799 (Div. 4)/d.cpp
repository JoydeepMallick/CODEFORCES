...
void test(){
  ...
  //cout << h << ' ' << m << endl;
  int t = (time[0] - '0') * 600 + (time[1] - '0') * 60 + (time[3] - '0') * 10 + (time[4] - '0');//total time in minutes
  int t0 = t;
  int ans = 0;
  while (1) {
    t += x, t %= 1440;//adding x , maintaining 24hr format i.e till 24*60 min
    // t/600 ->first digit of hour , t%10 -> last digit of minute
    // t/60 %10 -> 2nd digit of hour, t%60 /10 -> first digit of minute
    if (t / 600 == t % 10 && t / 60 % 10 == t % 60 / 10)//palindrome check
      ans++;
    if (t == t0)//encountering first repetition
      break;
  }
  cout << ans << '\n';
}
