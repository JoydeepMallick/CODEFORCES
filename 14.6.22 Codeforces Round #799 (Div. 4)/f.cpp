...
#define fori(a, n) for (ul i = a; i < n; i++)
#define forj(a, n) for (ul j = a; j < n; j++)
#define fork(a, n) for (ul k = a; k < n; k++)
...
void test(){
  ...
  cin >> n;
  int a[n];
  vi req_num;//list of all required numbers to check 3SUM
  int freq_lastdigit[10] = {0};//stores frequency of last digits of each element of array
  fori(0,n)
  {
    cin >> a[i];
    if(freq_lastdigit[a[i]%10]++ < 3)//reason explained below
      req_num.push_back(a[i]%10);
  }
  /*
   * a[i]%10 means last digit of each element of a
   *
   * now we store freq of each last digit in freq_lastdigit
   *
   * and we again check if freq of a digit is less than 4 we can check sum else
   * we end up checking 3 same number sum more than once.
   * (Note :- please keep in mind the post increment operation and <3 conditon
   * here because 3 times a number is accepted based on the values in
   * freq_oflastdigit i.e. 0, 1, 2 hence 3 times at maximum sme number will be
   * placed in req_num)
   */
  bool works = false;
  fori(0, req_num.size())
  {
    forj(i+1, req_num.size())
    {
      fork(j+1, req_num.size())
      {
        int sum_lastdigits = req_num[i] +  req_num[j] + req_num[k];
        works|= (sum_lastdigits%10) == 3;
      }
    }
  }
