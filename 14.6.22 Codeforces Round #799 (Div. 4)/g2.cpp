...
void test(){
  ...
  int ans = 0;
  set<int> breaks;//equivalent to treeset in java(automaticslly sorts elements)
  breaks.insert(n-1);//so that upper_bound never crashes as it the end point
  fori(0,n-1)
  {
    if(a[i] >= a[i+1]*2)//the invalid case
      breaks.insert(i);
  }
  fori(0,n)
  {
    auto next = (breaks.lower_bound(i));//equivalent to treeset.ceiling()
    /*
...
https://www.geeksforgeeks.org/set-lower_bound-function-in-c-stl/
     */
    //cout << i << "-->" << *next << endl;
    if((*next)-i+1 > k)//if the no. of elements i till *next ismore than k its possible
      ans++;
  }
  cout << ans << endl;
}
