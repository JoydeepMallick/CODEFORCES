...
void test(){
  ...
  fori(0,n)
  {
    cin >> a[i];
    sc[i+1] = sc[i];
    if (i && (a[i] << 1) > a[i-1])//when i not 0 and 2*a[i]>a[i-1]
      ++sc[i+1];
  }
  /*
   *Key observation

   any two adjacent pairs taken let 2^2 ai+2 < 2^3 ai+3

   we can simplify it to 1* ai+2 < 2* ai+3
                   =>   0.5*ai+2 < ai+3
   
   generalising it to 0.5 * ai+j < ai+j+1

  solution :-

  we make a list of all the places where the coupling condition is false and then we can avoid these places.
   */
  int ans = 0;
  fori(1, (n-k)+1)
  {
