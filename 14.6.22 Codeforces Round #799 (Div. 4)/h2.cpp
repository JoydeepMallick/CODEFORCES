...
void test(){
  ...
  cin >> n;
  int x[n];
  pii a[n+1];//an array of type pair contining n+1 elements
  fori(0,n)
  {
    cin >> x[i];
    a[i+1].first = x[i];//first part stores the element
    a[i+1].second = i;//second part stores index 
  }
  sort(a+1, a+(n+1));//sort by first element of pair in range from index 1 to n
  int lf = 0, res = INT_MIN, c = 0, nmin = INT_MAX;
  fori(1,n+1)
  {
    c+=2;
    if(nmin > (c - a[i].second - 2))
    {
      lf = a[i].second;
    }
    nmin = min(nmin, c - a[i].second -2);

    if((c - a[i].second -nmin) > res)
    {
      b = a[i].first;
      l = lf;
      r = a[i].second;
    }
    res = max(res, c - a[i].second - nmin);
    if(i == n || (i < n && a[i].first != a[i+1].first))
    {
      nmin = INT_MAX;
      c = 0;
    }
