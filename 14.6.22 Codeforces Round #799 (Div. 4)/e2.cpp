...
void test(){
  ...
  else
  {
    ...
    while(p2 <= n && p1 < p2)
    {
      if(num[p2] - num[p1] == s)
        ans = min(ans, n - (p2-p1));
      if(num[p2] - num[p1] <= s && p2 < n)//if diff less than s increase window
        p2++;
      else//decrese window size if diff more than s
        p1++;

      if(p2 == n && num[p2] - num[p1] < s)
        break;
    }
    if(num[p2] - num[p1] == s)
        ans = min(ans, n - (p2-p1));
