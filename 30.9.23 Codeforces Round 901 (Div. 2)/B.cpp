...
void test(){
    ...
    //either odd k then jelly fish = sum(a) - min(a) + max(b)
    //either even k then jelly fish = sum(a) - max(a) + min(b)
/* dbg(a,b); */
    ll mxa = MAX(a), mxb = MAX(b);
    ll mna = MIN(a), mnb = MIN(b);
/* dbg(mxa, mxb, mna, mnb); */
    ll s = 0;
    if(k&1){
/* dbg(accumulate(all(a), s)); */
      s = accumulate(all(a), s) + ((mxb > mna) ? (mxb - mna) : 0);
    }
    else{
/* dbg(accumulate(all(a), s)); */
      //first step
      s =  accumulate(all(a), s) + ((mxb > mna) ? (mxb - mna) : 0);
      //now jelyfish has maxa and maxb if it was greater than mina else no change
      //now gellyfish will need to a move no matter what happened since it need to take max element of a now since even if a performed operation it took max from b else max a remained same meaning max(maxa, maxb) moved
      s = s - max(mxa, mxb) + min(mna, mnb);
    }
    cout << s << endl;
}
