...
void test(){
  ...
  vll leastvalidL(n, -1); // for each index we compute the least possible L, anything below that is always valid but at least it must be the maximum value l provided can have to be valid 
  fori(1,n){
    leastvalidL[i] = ((a[i-1] != a[i]) ? i-1 : leastvalidL[i-1]);
  }

  while(q--){
    ll l,r;
    cin >> l >> r;
    l--;r--; //o based indexing
    ll i,j;
    if(l > leastvalidL[r]){
      i = -1, j = -1;
    }
