...
/* vector<int> a(1000006, 0), b(1000006, 0); */
...
int presentinboth = 0;
bool notfound = false;
int foundina = 0, foundinb = 0;
fori(1, k+1){
if(a[i] & b[i]) presentinboth++;
else if(a[i]) foundina++;
else if(b[i]) foundinb++;
else{
notfound = true;
break;
}
...
if(foundina > (int)(k/2) || foundinb > (int)(k/2)) no;
else yes;
}
}
