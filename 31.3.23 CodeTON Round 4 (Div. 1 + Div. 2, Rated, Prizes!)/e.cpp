...
void test(){
    ...
    int a[n+1];
    fori(1,n+1) cin >> a[i];
/* print(a, n+1);//avoid index 0 value */
...
/* } */
    fori(1,n+1){
        if(a[i]==0 && !vis[n]){//if weight of vertex is 0 then only check 
            if(has_spanning_tree(i)){
                yes;
                return;
            }
        }
...
The above code solves the problem of finding whether a given graph is a spanning tree of a given graph, where the weights of some vertices are 0.

In particular, the code reads in the number of vertices n and edges m of the graph, the weight of each vertex a[i] and the edges of the graph. For each vertex i with a[i] equal to 0, the code checks if the connected component containing i forms a spanning tree of the given graph.

To check if the connected component is a spanning tree, the code uses a modified version of a breadth-first search algorithm. It maintains a set of vertices ordered by their weights, and processes the vertices in order of increasing weight. The algorithm terminates when the weight of the current vertex is greater than the depth of the search, or when all vertices have been processed. If the algorithm has processed all vertices, and the number of vertices processed equals the number of vertices in the connected component, then the connected component forms a spanning tree of the graph.
 */
int main(){
