...
void test(){
    ...
    cin >> n >> c >> d;
    set<int> distinctnum;
    ull unavoidablecost = 0;
    /*
Firstly, we need to remove numbers such that each number appears at most once, this part of cost is unavoidable. Then, let's sort the array a1,a2…am (1≤ai<a2<......<am).
     */
    fori(0,n){
        ull ai;
        cin >> ai;
        if(distinctnum.find(ai) == distinctnum.end()) distinctnum.insert(ai);
        else unavoidablecost++;
    }
    unavoidablecost *= c;
    dbg(distinctnum, unavoidablecost);
    /*
...
.
     */
    ull ans = 2e18;//max possible value 
    int distinct_size = distinctnum.size();
    int i = 1; //index
    for(auto it : distinctnum){
        dbg(it, i, (it-i)*d);
        dbg(distinct_size, i, (distinct_size - i)*c);
        dbg((it - i)*d + (distinct_size - i)*c);
        ans = min(ans, 1ULL*(it - i)*d + 1ULL*(distinct_size - i)*c ); 
        // if set(2,3)   2 in starting 1 needs to be added i.e. (2-1)*d  
        // (2-1)*c
        i++;
    }
    //suppose all elements removed and only 1 added and it gave min cost.
    dbg(distinct_size*c + d);
    ans = min(ans, 1ULL*distinct_size*c + d);
    cout << ans+unavoidablecost << endl;
}
