...
void test(){
  ...
  /*
Let's rewrite the given equation: al+al+2+al+4+⋯+ar=al+1+al+3+⋯+ar−1 as
al−al+1+al+2−al+3+al+4+⋯−ar−1+ar=0.
...
or O(nlogn) depending on how you check if two elements of an array are equal. Be careful about using hash tables, as they can be hacked.

    Assume for following array :-
indices --> 0    1  2  3  4   5  6 
            11   1  3  2  3   4  7
  pref-> 0  11   10 13 11 14  10 17 
             ^   ^     ^       ^
             |   |_____|_______|
             |_________|

             hence 2 ranges :-
            1. after index 0 till index 3  i.e. [1..3]
            2. after index 1 till indec 5  i.e. [2..5]
  */
  set<ll> distinct;
  vll pref(n+1, 0);
  distinct.insert(pref[0]);
  fori(0,n){
    pref[i+1] = pref[i] + a[i]*((i%2) ? 1 : -1);
    distinct.insert(pref[i+1]);
  }

  dbg(pref);

  if(distinct.size() < n+1) yes;
  else no;
}
