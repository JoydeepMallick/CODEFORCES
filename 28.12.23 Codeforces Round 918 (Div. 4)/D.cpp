...
void recurfun(string s, ll i){
  if(i == -1){
    return;
  }
  else if(s[i] == 'a' || s[i] == 'e'){
    recurfun(s, i-2);
    cout <<((i-2 == -1) ?  "" : ".")<< s[i-1] << s[i];
  }
  else{
    recurfun(s, i-3);
    cout <<((i-3 == -1) ?  "" : ".")<< s[i-2] << s[i-1] << s[i];
  }
}
...
vector<string> syl;
/* string ans =""; */
 //go reverse,  if v then jump -2 and if c jump -3 
 ll i  = n-1;
 while(i > -1){
   if(s[i] == 'a' || s[i] == 'e'){
     //ans = "." + s.substr(i-1,2) + ans;
/* ans = "." + string(1, s[i-1]) + string(1, s[i]) + ans; */
     syl.push_back(s.substr(i-1,2));
     i -= 2;
   }
   else{
     //ans = "." + s.substr(i-2,3) + ans;
/* ans = "." + string(1,s[i-2]) + string(1, s[i-1]) + string(1, s[i]) + ans; */
     syl.push_back(s.substr(i-2,3));
     i -=3;
   }
 }
/* forj(1, ans.size()) cout << ans[j]; */
/* cout << endl; */
 rfor(i, syl.size()-1, -1){
   cout << syl[i];
   if(i > 0) cout << ".";
...
//NOTE : strange that the other logics give TLE since for big string the operation is heavy on system.
}
