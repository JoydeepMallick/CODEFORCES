...
void test(){
   ...
   // if already good return 0
    if(s%2 == 0) {
      cout << 0 << endl;
      return;
    }
    //even number of odds  sum up to even and even sum even so.....
    //only case of odd sum is when odd no. of odd elements are present
    int c = 0,cnt = INT_MAX;
    /*
we can either divide an odd and get an even else divide an even and try to get an odd
to solve the problem whichever gives minimum steps
     */

    //find the minimum number of times a number needs to be divided to change
    //it parity ie even to odd or odd to even
    for(int ele : a){
      if(ele%2){
        while(ele%2) {
          ele/=2;
