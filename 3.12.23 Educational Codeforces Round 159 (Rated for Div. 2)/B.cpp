...
void test(){
  ...
  ll maxtasksavailable = ceill(n/7.0);
/* dbg(maxtasksavailable); */
  

  ll daysoff = n;
/*
 * breute will give tle 
 *
 *
  while(pointsgathered < P){
    ll maxpoints = l + ((twotaskspicked > 0) ? 2*t : ((onetaskextra > 0)? t : 0) );
    pointsgathered += maxpoints;
    daysoff--;
    if(twotaskspicked > 0) twotaskspicked--;
    else if(onetaskextra > 0) onetaskextra =0;
  }
*/


  // if 2 tasks + lesson a day for no of tasks /2 days is enogh it will be so, else complete all tasks following the strategy and the remaining days by lessons as needed.
  ll daysworked = max( ceill(P/(1.0 * (l + 2*t))) , ceill((P - maxtasksavailable*t) / (1.0 * l)) ) ;
  daysoff -=daysworked;
  cout << daysoff << endl;


  //instead of ceil (x/y) we can also do (x-1)/y + 1
}
