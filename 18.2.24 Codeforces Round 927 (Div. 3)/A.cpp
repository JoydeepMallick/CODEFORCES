...
ll f(string &s, ll ind, ll n, vll &dp){
  if(ind > n-1) return 0;
  if(ind == n-1){
    return  s[n-1] == '@';
  }
  ...
  if(s[ind] == '*') return 0;
  //move 1 step 
  ll one = (s[ind] == '@') + f(s, ind+1, n, dp);
  ll two = (s[ind] == '@') + f(s, ind+2, n, dp);
...
void test(){
  ...
  //moving 1 step until possible is greedy else moving 2 steps if possible 
  ll ans = 0;
  vll dp(n, -1);
  ans = f(s, 0, n, dp);
  cout << ans << endl;
