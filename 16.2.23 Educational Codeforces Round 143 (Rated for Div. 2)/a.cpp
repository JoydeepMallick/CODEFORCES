...
bool checkalternate(int n, string s, int &c){
    //c = 0 denotes no  adjacent same element occured
    //c = 1 denotes a freqeuncy in alternate exists
    for(int i = n-1; i > 0; i--){
...
void test(){
  ...
  cin >> s1 >> s2;
  /*yes only if :-
   *    top of towers if same both the towers must already be beautiful
   *
   *    else if top of towers different then :-
   *    1.  any one tower at max can two side by side elements same only for one element.
   *    2. other all elements will folow alternate order.
   */
  int c = 0;
    if(checkalternate(n,s1,c)) {if (checkalternate(m,s2,c) ) yes;
        else no;}
    else no;
}
