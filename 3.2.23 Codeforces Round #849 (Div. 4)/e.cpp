...
void test(){
  ...
  cin >> n;
    ...
    /*
     * operations allowed swap of signs of adjacent element
     *
     * hence beneficial to swap to negative adjacent elements or a negative and a 0
     *
     * and if odd no. of negative elements present then we simply add all number as absolute values and subtract the min of the abosulte values twice so that one goes in cancellation and other goes in reduction.
     *
     *
     * the operation will ensure that we can shift the negative sign to the smallest element by repeated swapping of signs with adjacent elements hence this case is taken care of .
     */
    ull ans = 0;
    int min_ele = INT_MAX,cnt_neg = 0;
    fori(0,n){
        if(a[i] < 0) cnt_neg++;
        a[i] = abs(a[i]);
        ans += a[i];
        min_ele = min(min_ele, a[i]);// min of abs value , try to get a zero else the min to maintain max sum
    }
    if(cnt_neg & 1){
        ans = ans - 2*min_ele;
    }
