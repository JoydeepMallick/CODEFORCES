...
void test(){
  ...
  /*
   * we need to find the total cost for using a teleporter at a given index. i.e. i + ai hence lesser the value of i+ai more the number of teleporter can be used.
   *
   * we can hence store the cost in an array and then simply calculate the the number of values before which c becomes 0
   */
/*
 * can use either 
 * priority_queue<ll, vll, greater<ll>> minheap;
 *
 * or maxheap i.e. normal priority queue and to get sorted in ascending simply adda negative sign to each element and process accordingly.
 *
 * or any thing that needs to be sorted.*/

    int mxtele = 0;
    auto val = cost.begin(); 
    dbg(cost);
    while(c >= *val && val != cost.end()){
        c -= *val;
        val++;
        mxtele++;
    }
