...
void test(){
  ...
  fori(1,k+1) cin >> cold[i];
  fori(1,k+1) cin >> hot[i];
    
  int lastoncpu1 = 0;
  int lastoncpu2 = 0;
  // 2 cpu is similar to page table of size 2
  int time = 0;
  int lru = 1;//intially assume cpu1 was least recently used 
  fori(1, n+1){
      int currentprocess = a[i];
      if(lastoncpu1 == currentprocess || lastoncpu2 == currentprocess){
          //if the process to be eecuted already ran on any of the cpus then hot 
          time += hot[currentprocess];
      }
      else{
          //use the least recently used cpu i.e. which was not used last 
          if(lru == 1) lastoncpu1 = currentprocess;
          else lastoncpu2 = currentprocess;
          time += cold[currentprocess];
          lru = (lru == 1) ? 2 : 1;
      }
  }
  cout << time << endl;
