...
void test(){
    ...
    cin >> n;
    int a[n+1];
    //for the sum to be odd atleast one number in the sum has to be odd
    int oddind = -1;
    bool possible=  false;
    vi oddnoindices, evennoindices;
    fori(1, n+1){
      cin >> a[i];
      if(a[i]%2) {
        oddnoindices.push_back(i);
        possible = true;
        oddind = i;    
      }
      else evennoindices.push_back(i);
    }
    if(possible){
      //print two indices  of either even numbers else odd numbers(avoid theodd index) but not mixed 
      if(evennoindices.size()>=2) {
        auto it = evennoindices.begin();
        yes;
        cout << *it << " " << *(it+1) << " " << oddind << endl;
      }
      else if(oddnoindices.size() >= 3){
        auto it = oddnoindices.begin();
        yes;
        int t1 = (*it != oddind)? *it : *(++it);
        it++;
        int t2 = (*it != oddind)? *it : *(++it);

        cout << t1 << " " << t2 << " " << oddind << endl;
