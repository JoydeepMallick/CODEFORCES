...
/* 1, 1*2, 2*2, 4*2, ... 3, 3*2, 6*2,....., 5, 5*2,..... */
 ...
 /*   
    ...
    do{
        ...
        for(int i =0; i < n; i++){
            int g = __gcd(a[i], a[(i+1)%n]);
            if(present.find(g) == present.end()){
                present[g] = 1;
                d++;
            }
        }
    ...
    dbg(ans);
    for(auto v : store[ans]){
        print(v);
    }
   */

    //brute force observation max distinct gcd is always half of the number of elements
}
