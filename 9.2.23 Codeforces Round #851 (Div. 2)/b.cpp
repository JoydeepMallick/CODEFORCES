...
void test(){
  ...
  cin >> n;
    int l = 0, r = n, mid;
   //perform binary search from middle till the element is found
    while(l <= r){
        mid = l + (r-l)/2; 
        if(abs(sumofdigits(mid) - sumofdigits(n-mid)) <= 1){
            cout << mid << " " << n-mid << endl;
            break;
        }
        else if(sumofdigits(mid) < sumofdigits(n-mid)){
            l = mid + 1;//go to right half
            //as sum of mid - sum of (n-mid) was less than -1 then we need to increase mid
        }
        else{
            r = mid-1;//go to left half
            //as difference is +1 we need to decrease mid
        }
    }
}
