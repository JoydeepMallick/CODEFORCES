...
/* array<int, n> a{}; */
  ...
  fori(1,n+1){
    cin >> a[i];
    if(a[i]==2) posof2.push_back(i);
  }
    //if freq of 2 is even then print mid index wrt to 2 else -1
    if(posof2.size()%2) cout << -1 << endl;
    else cout << ((posof2.size()) ? posof2[posof2.size()/2 - 1] : 1)<< endl;
}
