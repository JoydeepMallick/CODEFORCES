...
void test(){
  ...
  cin >> n;
    /*
     * for a pair ot satistify such property n must always be odd else the difference between si and si+1 can never be 1 
     *
     * si+1 - si = 1 given and s1 < s2 < s3 .....
     *
     *
     * for n  = 3
     *
  ...
  if(n&1){
    yes;
    ll s = 2*n + 1 - n/2;//starting or min sum
    ll tmp;
    fori(1, n+1){
        if(i&1){
            tmp = (i+1)/2;
