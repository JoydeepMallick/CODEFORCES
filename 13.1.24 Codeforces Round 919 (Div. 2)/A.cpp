...
void test(){
  ...
  cin >> n;
  ll a,x;
  ll minleft = LONG_MIN, maxright = LONG_MAX;
  set<ll> del;
  ll cntdel = 0;
  fori(0,n){
    cin >> a >> x;
    if(a==1) minleft = max(minleft, x);
    else if(a==2) maxright = min(maxright, x);
    else del.insert(x);
  }
  for(auto num : del){
    if(num >= minleft && num <= maxright) cntdel++;
  }
  dbg(minleft, maxright, cntdel);
  if(minleft > maxright) cout << "0\n";
  else cout << maxright - minleft + 1 - cntdel << endl;
