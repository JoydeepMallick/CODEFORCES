...
void test(){
  ...
  ll ans = LONG_MIN;
  dbg(pref);
  //remove elements one by one form maximum 
  ll i = 0;
  while(i <= k && n-i >= -1){
    ll diff = pref[n-i] - pref[max(0ll,n-x-i)];//element from index n-i+1 till n-1 if valid removed
    ll s = (pref[max(0ll,n-x-i)] - diff)  ;
    if(n-i == -1) s = 0; //all deleted
    ans = max(s, ans);
    dbg(diff, s, n-x-i, n-i, ans);
    i++;
  }
  cout << ans << endl;
