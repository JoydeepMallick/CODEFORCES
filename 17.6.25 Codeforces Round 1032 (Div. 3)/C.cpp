...
void test(){
    ...
    fori(0, n){
        forj(0, m){
            ...
            else if(a[i][j] > maxele){
                maxele = a[i][j];
                maxeletotalcnt = 1; // reset the count
            }
        }
    }

    //count all number of max elements per row 
    
    //count the number of max elements per column
    ...
    //for each r and c check if total number of maxelement is equal to all elements in that row and column.
    //edge case of subtraction to be handled for case when max element lies at r,c since it gets counted twice

    bool canbereduced = false;
    fori(0,n){
        forj(0,m){
            int maxelecntRC = rowcnt[i] + colcnt[j] - (a[i][j] == maxele);
            if(maxelecntRC == maxeletotalcnt){
                canbereduced = true;
                break;
            }
        }
