...
void test(){
    ...
    fori(0, n-1){
        forj(0, n-i-1){
            if(a[j] > a[j+1]){
                swap(a[j], a[j+1]);
                steps.push_back({1, j+1}); // 1 for a, j+1 is the index in 1-based indexing
            }
        }
    }

    //bubble sort on b n^2 STEPS
    fori(0, n-1){
        forj(0, n-i-1){
            if(b[j] > b[j+1]){
                swap(b[j], b[j+1]);
                steps.push_back({2, j+1}); // 2 for b, j+1 is the index in 1-based indexing
            }
        }
    }

    //swap ai and bi when ai > bi since 
    //a = 1....n 
    //b = n+1....2n
    fori(0, n){
        if(a[i] > b[i]){
            swap(a[i], b[i]);
            steps.push_back({3, i+1}); // 3 for swap, i+1 is the index in 1-based indexing
        }
    }
