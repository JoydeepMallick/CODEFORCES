...
void test(){
  ...
  string s;
  cin >> s;
  int start = -1, end = -1, r = 0, b = 0;
  bool encounter = false;//counts no of times we encounter RB subsequnce
  fori(0,s.size()){
    if(s[i]=='W') {
      if(encounter){
        //after subsequnce passes away if at least one R and B is not present
        encounter = false;
        if(r == 0 || b == 0) break;
      }
      start = -1, end = -1;
      ...
      if(start==-1){
        encounter = true;
        r = 0, b = 0;//for new subsequnce initialised again
        start = i,end = i;
        if(s[start]=='R') r = 1;
        else b = 1;
      }
    ...
    //dbg(s[i],start,end,r,b);
  }
  if(r == b) yes; //both 1 signifies yes, both 0 signlifies all white
  else no;
}
