...
void test(){
    ...
    //array must be increasding or decreasing else not possible

    int incrdiv = INT_MAX, decrdiv = INT_MAX;
    fori(0, n){
        incrdiv = min(incrdiv, (int)(a[i]/(i+1)));    
        decrdiv = min(decrdiv, (int)(a[i]/(n-i)));
    }
    //reduce all 
    vector<int> incr = a, decr = a;
    fori(0, n){
        incr[i] -= ((i+1)*incrdiv);
        decr[i] -= (n-i)*decrdiv;
    }
    ...
    if(incr == all0 || decr == all0) {
        yes;
        return;
    }
    
    bool allincrdiv = true, alldecrdiv = true;
    fori(0, n){
        allincrdiv &= (incr[i] == (i+1) * incr[0]);
        alldecrdiv &= (decr[i] == (n-i) * decr[n-1]);
    }

    if(allincrdiv || alldecrdiv) yes;
    else no;
