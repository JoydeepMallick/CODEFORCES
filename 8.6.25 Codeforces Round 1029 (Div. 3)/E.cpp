...
void test(){
    ...
    read(a);
    read(b);
    //read the solution provided in editorial
    /*
    Bigger picture
     ai can be set to bj for j > i
     ai can be set to aj fro j > i+1
    ...
    Similarly for bi 
     bi can be set to aj for j > i
     bi can be set to bj for j > i+1
    ...
    This is all possible due deletion of i+1 index element

    ai can never be set to ai+1 and bi can never be set to bi+1

    From here we can see that

      ai and bi can be set to either ai+2 or bi+2 (criss cross)

      Hence including i all previous indices can be made same. 


      Now one edge case occurs when 
         ai ai+1 ai+2 ai+3 .......
         bi bi+1 bi+2 bi+3 .......
              
         if ai = ai+1 or bi = bi+1 then we can set 
         bi = ai+1 i.e. ai 
          or
         ai = bi+1 i.e. bi
  ...
  int lastindmatch = -1;
  set<int> s; // keep track of already visited elements from both so whenever a match found in both arrays we stop, no need to differntiate between a and b since both array elements can be used to set.
  rfor(i, n-2, -1){
    if(a[i] == b[i] || a[i] == a[i+1] || b[i] == b[i+1]){
      lastindmatch = i;
      break;
    }else{
      if(s.find(b[i]) != s.end() || s.find(a[i]) != s.end()){
        lastindmatch = i;
        break;
      }
    }
    //maintaining a distance of i+1 to ensure we do not assume cases like
    // a[i] = b[i+1] or b[i] = a[i+1] as valid 
    s.insert(a[i+1]);
    s.insert(b[i+1]);
  }
  ans = lastindmatch + 1;
