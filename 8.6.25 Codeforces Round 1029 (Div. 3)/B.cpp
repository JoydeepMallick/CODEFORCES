...
void test(){
    ...
    // hence answer = n-8 n-5 n-4 n-1 n n-2 n-3 n-6 n-7
    vector<int> perm(n);
    ...
    int topind;
    if(lvls&1){
      //for even length right tilted
      topind = (n-1)/2;
    }else{
      //for even length left tilted
      topind = n/2;
    }
    perm[topind] = n;
    int lt = topind-1, rt = topind+1;
    int val = n;
    while(lvls--){
      dbg(n, lt, rt, val);
      if(lt >= 0) {
        perm[lt] = (goLeft ? val-1 : val-2);
        if(perm[lt] == 0) perm[lt] = 1;
      }
      if(rt < n) {
        perm[rt] = (goLeft ? val-2 : val-1);
        //special case
        if(perm[rt] == 0) perm[rt] = 1;
      }
      lt--;
      rt++;
      val -= 2;
