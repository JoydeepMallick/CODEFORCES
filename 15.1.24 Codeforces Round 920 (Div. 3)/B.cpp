...
void test(){
  ...
  string s, f;
  cin >> s >> f;
  ll cntmismatch = 0, cnts = 0, cntf = 0;
  fori(0,n){
    if(s[i] != f[i]) cntmismatch++;
    if(s[i] == '1') cnts++;
    if(f[i] == '1') cntf++;
  }
  if(cnts == cntf){
    //shift if needed per day 
    cout << cntmismatch/2 << endl;
  }
  else{
    ll toaddrem = abs(cnts - cntf);
    ll toshift = cntmismatch - toaddrem;
    cout << toaddrem + toshift/2 << endl;
  }
