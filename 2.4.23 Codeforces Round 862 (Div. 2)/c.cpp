...
/* print(kx,n); */
    fori(0,m){
        ll a,b,c;
        cin >> a >> b >> c;
/*This code is using the lower_bound algorithm provided by the C++ Standard Library to search for an element in a sorted range and return its index. Specifically, it searches for the first element in the range lines.begin() to lines.end() that is greater than or equal to b. The result is then the difference between the iterator returned by lower_bound and the beginning iterator of the range lines.begin(), which gives the index of the element in the range.

In other words, this code finds the index of the first element in a sorted vector lines that is greater than or equal to a given value b.*/
        int ind = lower_bound(kx, kx+n, b) - kx;
        dbg(ind);
        ...
        if(ind < n && (ll)pow(kx[ind]-b, 2) < 4*a*c){// valid index and if smallest greater counter part of b to right
            yes;
            cout << kx[ind] << endl;
        }
        else if(ind > 0 && (ll)pow(kx[ind-1]-b, 2) < 4*a*c){ // if valid index then only check for left counterpart of b i.e. greatest number not greater than b
            yes;
            cout << kx[ind-1] << endl;
        }
        else no;
