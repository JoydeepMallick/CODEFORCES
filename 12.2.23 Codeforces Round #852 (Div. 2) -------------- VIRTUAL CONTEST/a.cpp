...
void test(){
  ...
  cin >> a >> b >> n >> m;
  if(n <= m){
      //m doesn't matter since quantity needed  is less
      cout << (ull)(n*min(a,b)) << endl;
  }
  else if(a > b && a*m <= b*(m+1)){
      //if price of a is more than b its beneficial only if the the price of m items <= price of m+1 items at rate b
      //also given that for each m items 1 kilo extra
      ull timesm = n/(m+1);
      cout << (ull)(a*m*timesm + (n - (m+1)*timesm )*b) << endl;
  }
  else{
      ull timesm = n/(m+1);
    cout << (ull)(a*(n-timesm)) << endl;
  }
}
