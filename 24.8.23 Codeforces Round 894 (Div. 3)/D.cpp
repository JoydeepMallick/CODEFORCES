...
void test(){
    ll n;
    cin >> n;
    //x is the number of flavours which mix with themselves and y are the falvours which intermix
    ll D = (1 + sqrtl(1+8*n))/2; // posstive root for x+y
    //to maximize the output we need to D number of flavours + DC2 != n then remaining were made by mixing same flavours which need to be counted only once. 
    cout << D + n - D*(D-1)/2 << endl;
