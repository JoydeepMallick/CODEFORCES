...
template<typename T>
void __print(const vector<T>& v) {cerr << "{ "; for (auto x : v) { cerr << x << " ";} cerr << " }"; }
...
void test(){
    ...
    string s;
    cin >> s;
    // if a character at i then possible allowed postions only i + x where x = 2,4,6,....
    // ie not possible if x = 1, 3, 5  so check
    //
    //element at i, i+1, i+3,...... must all be distinct.
    //
    //create a map of vectors of indexes of characters
    //
    //codeforces
    //
    //c {0,7}  o{1,5}  d{2}  e{3,8}  f{5}  r{6} s{9}   avoid lone elements   so  only c , o, e  
    //
    
    unordered_map<char, vi> charind;
    fori(0,n){
        charind[s[i]].pb(i);
    }
/* dbg(charind); */
    ...
    for(auto it : charind){
        if(it.second.size() > 1){
            //parity of index must be same 
            fori(1, it.second.size()){
                if((it.second[i] & 1) != (it.second[0] & 1)){
                    possible = false;
                    break;
                }
            }
