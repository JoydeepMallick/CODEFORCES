...
void test(){
  ll a,b,c;
  cin >> a >> b >> c;
 //if all of same parity tyhen all 1 
//else if 2 odd and 1 even then the even parity psotion is 1 
//similarly if 2 even and 1 odd then odd parity postion si 1 

  if((a&1) == (b&1) && (b&1) == (c&1)){
      cout << "1 1 1\n";
  }
  else{
        int oddcount = (a&1) + (b&1) + (c&1);
        dbg(oddcount);

        if(oddcount == 1){
            //print 1 where odd number is present
            if(a&1) cout<< "1 0 0\n";
            else if(b&1) cout << "0 1 0\n";
            else cout << "0 0 1\n";
        }
        else{
            //print whre even number 
            if((a&1) == 0) cout<< "1 0 0\n";
            else if((b&1) == 0) cout << "0 1 0\n";
            else cout << "0 0 1\n";
