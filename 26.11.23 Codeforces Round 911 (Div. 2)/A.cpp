...
void test(){
  ...
  //# are partitions if dot count between them is 2 or 1 then all 1 operations only.
  //But if amy such partition has dot count >= 3 then only 2 one op will be suffiecnt rest water can be generated infinitely.
  ll cnt =  0;
  vll part_size;
  fori(0,n){
    if(s[i] == '#' && cnt != 0){
    ...
    if(i == n-1 && cnt > 0) part_size.pb(cnt);
  }

  dbg(part_size);

  if(part_size.size() > 0 && MAX(part_size) >= 3) cout << "2\n";
  else{
    ll sum = 0;
    sum = accumulate(all(part_size), sum);
    cout << sum << endl;
  }
