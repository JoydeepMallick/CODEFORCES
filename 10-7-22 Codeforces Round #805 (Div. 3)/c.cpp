...
void test(){
  ...
  cin >> n >> k;
  int u[n];
  map<int,pii> ind;//stores highest and lowest index of vlaues
  fori(0,n){
    cin >> u[i];
    if(ind.count(u[i])) ind[u[i]].second = i;
    else {
      ind[u[i]].first = i;
      ind[u[i]].second = i;
    }
  ...
  fori(0,k){
    int a,b;
    cin >> a >> b;
    //if both a and b exists and lowest index of a must always be less than or
    //equal to highest index of  b then yes else no(other indices doesnt matter)
    if(!ind.count(a) || !ind.count(b) || ind[a].first > ind[b].second) no;
    else yes;
  }
}
