...
void test(){
  ...
  int p;
  cin >> p;
  //remove the costliest letters first till the cheapest.
  string w(s);//stores copy of string s in w
  sort(allrev(w));//sort in descending order
  //total cost calculation
  int cost = 0;
  fori(0, s.length()) cost += s[i] - 'a' + 1;//s[i] - 65 + 1
  //cost reduction and finding out the elements from decreaingly sorted w to be
  //deleted i.e elements with highest cost deleted first
  map<char, int> del;
  fori(0, w.length()){
    if(cost > p){
      del[w[i]]++;
