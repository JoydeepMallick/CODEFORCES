...
void test(){
  string s;
  cin >> s;
  //remember maximum 3 letters a day
  char letter[3] = {'\0' ,'\0' ,'\0'};
  int c  = 1;//best case
  for(char let : s){
    //dbg(letter[0], letter[1], letter[2]);
    if(!letter[0]) letter[0] = let;
    else if(!letter[1]){
      if(let != letter[0]) letter[1] = let;
      else continue;
    }
    else if(!letter[2]){
      if (let != letter[0] && let != letter[1]) letter[2] = let;
      else continue;
    }
    else if(let != letter[0] && let != letter[1] && let != letter[2]){
  ...
  //if(letter[0] || letter[1] || letter[2]) c++;
  cout << c << endl;
}
