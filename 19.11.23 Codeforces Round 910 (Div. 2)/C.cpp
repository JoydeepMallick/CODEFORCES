...
void test(){
  ...
  if(diff < 0 || diff&1) no;
      ...
      fori(0,n){
          forj(0, m-1){
              cout << "RB"[j&1] << " \n"[j == m-2];//alternate coloring all horizontal lines
          }
      }

      fori(0,n-1){
          forj(0,m){
              cout << "RB"[ (i==0 && j==0) || (i+j)&1 ] << " \n"[j == m-1] ;//either starting 2 left or ending 2 right will be same color, I have taken starting left 2 to be same 
          }
      }
  }
