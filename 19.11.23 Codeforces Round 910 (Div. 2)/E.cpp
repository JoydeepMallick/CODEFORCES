...
void test(){
  ...
  fori(0,m){
      ...
      if(vec[x].empty()){
          ...
          //take out the first index in a where char X occured
          int p = vec[x].back();
          vec[x].pop_back();
          //for all aphabets present in  a and less than p of b 
          forj(0, x+1){
              while(!vec[j].empty() && vec[j].back() < p){
                  vec[j].pop_back(); // delete that index meaning it can be smoothly shifted to desired location
              }
          }
      }
  }
