...
//observation try to break loking at next element and best breaking method is n/2 and n-n/2 
//continue such breakage until last element matches the next 

ll stepsbroken(ll curele,ll nextele){
    return (curele - 1)/nextele;
}
...
void test(){
  ...
  ll ans = 0;
  rfor(i,n-2, -1){
    ll stepsneeded = stepsbroken(a[i], a[i+1]);
    ans += stepsneeded;
    dbg(a, stepsneeded);
    ll leftmostelenow = a[i]/ (stepsneeded+1);
    a[i] = leftmostelenow; // actual right of a[i-1] is this now 
  }
