...
void test(){
    string a,b;
    cin >> a >> b;
    //if atleast 2 consequtive letters match in a and b then it is safely asterisk minor as we can replace rest with 2 asteriks or 1 as per need or even no asterisk.
    //  if a == b then no asterisk
    // else if only one letter of a and b match and its either the first of both or last of both then only possible.
    
    if(a==b || a[0]==b[0] || a[a.size()-1] == b[b.size()-1]){
        yes;
        if(a==b) cout << a << endl;
        ...
        //at least 2 consecutive must be same 
        fori(0, a.size()-1){
            string slice = a.substr(i, 2);
            forj(0,b.size()-1){
                dbg(slice, b.substr(j,2));
                if(b.substr(j,2) == slice){
                    yes;
                    cout << "*" << slice << "*\n";
                    return;
                }
        ...
        //now we can surely tell such cant exist
        no;
    }
}
