...
void refresh(){
  for(auto it : pref) if(it.se == 1) pref[it.fi] = 0;
}
...
void test(){
  ...
  read(a);
/*
 // find prefix sum and map them in an array 
  ll sum = 0;
  vll prefsum;
  fori(0,n){
    sum += a[i];
  ...
  while(q--){
      ...
      if(op == 1){
        ...
        if(s > prefsum[n-1]) no;
        ...
        else if(pref[s] == 1) yes;
        
        else{
            ll ind = n-1;
            bool exists = false;
            while(prefsum[ind] > s){
                ll tosubtract = prefsum[ind] - s;
                dbg(a[ind], tosubtract, pref[tosubtract]);
                if(pref[tosubtract] == 1){
                    yes;
                    exists = true;
                    break;
                }
          ...
          //updation of prefix sum in minimum operations 
          forj(i-1, n){
              pref[prefsum[j]] = 0;//old sum no longer exists 
              prefsum[j] += diff; // update new sum 
              pref[prefsum[j]] = 1; // update new sum 
          }
