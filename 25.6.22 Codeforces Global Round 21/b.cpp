...
void test(){
  ...
  fori(0,n)
  {
    ...
    else if(l!= -1 && r==-1 && a[i]==0) r = i-1;

    else if(l!= -1 && r != -1 && a[i] != 0)
      {
        r = -1;//need new r since non 0 hit after 0
        add = 1;
      }
    
    if(i == n-1) r = i;//for last non zero element r has to be i
  }
  if(l!=-1 && r!=-1)//some elements are definitely non zero
    c = 1;//minimum operation needed
  cout << c + add<< endl;// 1 more step due to existence of 0 in range of l to r
}
