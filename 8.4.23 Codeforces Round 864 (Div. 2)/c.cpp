...
void test(){
    int n,m;
    cin >> n >> m;
/* We can first ask (1,1) and get the result k */

/* . Obviously, the king must be on the following two segments: */

/* from (1,k+1) to (k+1,k+1)  a set of  column k+1 */

/* from (k+1,1) to (k+1,k+1).  a set of row of k+1 */
    /*
     above can be checked via drawing a checkboard since it always has movement in 8 directions always such a coordinate can be reached by k suitable moves.
...
, the king is at (k+1,k+1)
.
If p<k
, the king is at (p+1,k+1) its defintely in column k+1
.
If q<k
, the king is at (k+1,q+1). its definitely in row k+1
     */
    int dist = ask(1,1);
    if(dist >= n){
        int d2 = ask(1, dist+1);// defintely not in first row
        printans(d2+1 , dist + 1);
    }

    else if(dist >= m){
        int d2 = ask(dist+1, 1);//definitely not in last column
        printans(dist+1 , d2+1);
    }

    else{
        int d2 = ask(dist + 1, 1);
        int d3 = ask(1, dist + 1);
        if(dist == d2 && d2 == d3) printans(dist+1 , dist+1);
        else if (d3 == dist) printans(dist+1 , d2+1);
        else printans(d3+1 , dist+1); 
    }
