...
bool check_cor(int x, int y){
    vector<pair<int, int>> coord = {{1,1},{1,m},{n,1},{n,m} };
    for(auto it : coord){
        if(x==it.fi && y==it.se) return true;
    }
    return false;
}
...
void test(){
    ...
    //block one and job done. ans will either be 2, 3 , 4 
    //if any of the coordinates is at corner 
    if(check_cor(x1, y1) || check_cor(x2, y2)) cout << 2 << endl;
    else if (check_side(x1,y1) || check_side(x2,y2)) cout << 3 << endl;
    else cout << 4 << endl;
