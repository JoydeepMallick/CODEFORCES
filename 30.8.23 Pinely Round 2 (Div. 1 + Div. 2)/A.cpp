...
void test(){
    ...
    // c1 ->active users go to sleep and wake worst case 
    // c2 -> active users go to sleep but never read people wake best case 
    ll c1, c2;
    c1 = a;
    c2 = a;
    for(auto c : notif){
      if(c == '+'){
        c1++;
        if(c1 == n) break;// at least once all were online in worst case even
        if(c2 < n)c2++;
      }
      else{
        c1--;
