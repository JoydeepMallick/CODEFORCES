...
/*
  ...
  if(frA.size() == 1) (frB.size() >= 3) ? yes : no;
  else if(frB.size() == 1) (frA.size() >= 3) ? yes : no;
  else{
    //frequency of at least 2 elements in each must be >= 2 
    int cntA = 0, cntB = 0;
    for(auto it : frA) if(it.second >= 2) cntA++;
    for(auto it : frB) if(it.second >= 2) cntB++;
    if(cntA < 2 && cntB < 2) no;
    else yes;
  }
  */ 
