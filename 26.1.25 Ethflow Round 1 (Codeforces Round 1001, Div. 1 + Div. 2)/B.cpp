...
void test(){
    ...
    // for val max blocks to move either on left or right before returning = (val-1)/2
    // _ _ 5 _ _ 
    // _ 4 _ 
    // ie. val = 2 then max block = 0
    // val = 3 then max block = 1
    // val = 4 then max block = 1 
    // val = 5 then max block = 2 
    // val = 6 then max block = 2
    // val = 7 then max block = 3
    // val = 8 then max block = 3 
    // val = 9 then max block = 4
    // .....

    fori(0, n){
        ll val = a[i];
        ll maxblocks = (val-1)/2;
        ll noofblocksonleft = i;
        ll noofblocksonright = n-i-1;
        if(noofblocksonleft > maxblocks || noofblocksonright > maxblocks){
            no;
            return;
        }
    }
