...
void test(){
    ...
    vll smallest, secsmallest;
    //only concerned with first 2 elements in sorted array
    fori(0,n){
        smallest.pb(a[i][0]);
        if(a[i].size() >= 2) secsmallest.pb(a[i][1]);
    }

    dbg(smallest, secsmallest);
    ll ans = 0;
    ans = accumulate(all(secsmallest), ans);
    ans += *min_element(all(smallest)) - *min_element(all(secsmallest));
