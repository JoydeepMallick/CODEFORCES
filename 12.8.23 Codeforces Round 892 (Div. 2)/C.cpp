...
void test(){
    ll n;
    cin >> n;
    //from brute force its clear that permutation is from 1 to some k and then n n-1 ..... k+1 in reversed fashion
    //calculation all such possible permutations sum ans then finding out max will take n^2 time which is ok for the given problem.
    ll ans = 0;
    
    fori(1, n+1){
      ll reverse_at = i, toput = n;
      ll tmp = 0, mx = 0;
      forj(1, n+1){
        ll value;
        if((ll)j < reverse_at) value = j*j;
        else value = j*toput--;
        mx = max(value, mx);
        tmp += value;
      }
...
/*
 //brute solution
    ...
    vll a;
    fori(1,n+1) a.pb(i);
    unordered_map<ll, vector<vector<ll>>> possibleperm;
    do{
      dbg(a);
      ll tmp = 0, mx = 0;
      fori(0,n){
      ...
      possibleperm[tmp-mx].pb(a);

      ans = max(ans, tmp-mx);
    }while(next_permutation(all(a)));

    cout << ans << endl;
    for(auto v : possibleperm[ans]){
      print(v);
    }
*/
...
7
1 3 2 
_____ TEST CASE #3 _____
17
1 2 4 3 
1 4 3 2 
_____ TEST CASE #4 _____
35
1 2 5 4 3 
_____ TEST CASE #5 _____
62
1 2 3 6 5 4 
_____ TEST CASE #6 _____
100
1 2 3 4 7 6 5 
1 2 3 7 6 5 4 
_____ TEST CASE #7 _____
152
1 2 3 4 8 7 6 5 
