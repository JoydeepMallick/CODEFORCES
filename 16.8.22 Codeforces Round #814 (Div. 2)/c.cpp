...
void test(){
    int n,q;
    cin >> n >> q;
    int a[n+1],fightcount,uniquefights;
    map<int,vi> wins; 
    int top;
    //wins[top] = 0;wins[n] = 0;
    fori(1,n+1){
        ...
        if(i == 1) top = a[i];
            ...
            fightcount = (i==1)?1:i-1;
            wins[n].push_back(fightcount);
            uniquefights = fightcount;
            dbg(uniquefights);
        }//it had to fight once and win to come to top
        else if(wins[n].size()) continue;//wins[a[i]] = 0;//after max element is reached no one can ever win
        else {
            if(top < a[i]) top = a[i]; 
            fightcount = (i==1)?1:i-1;
            wins[top].push_back(fightcount);//stores fight one has won
        }
    }
   
    //till the highest element comes to top we need to count  each win after
    //that no one except highest element wins(a is permutation of n)
    while(q--){
        int i,k;
        cin >> i >> k;
        if(k >= uniquefights) {
            if(a[i] == n) cout << wins[n].size() + k - uniquefights << "------" << uniquefights;
            else cout << wins[a[i]].size();
        }
        else{
            int c= 0;
            for(int whichfight : wins[a[i]]){
                if(k < whichfight){
                    c++;
                }
                else{
                    if(k == whichfight) cout << c+1;
                    else cout << c;
                    break;
                }
            }
