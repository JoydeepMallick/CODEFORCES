...
void test(){
    ...
    vector<ll> diff(n-1); // store diff, first element of diff is a[0]
    diff[0] = a[0];
    fori(1, n-1){
        diff[i] = (a[i] - a[i-1]) ;
    }
    ...
    //mark the numbers found from 1 till n 
    vector<bool> seen(n+1, false);
    for(auto num : diff){
      if(num <= n) seen[num] = true;
    }
/* dbg(seen); */

    // store all not found numbers 
    vll missing;
    fori(1,n+1){
      if(!seen[i]) missing.pb(i);
    }

    dbg(missing);
    if(diff.back() > n){
      //if max diff is more than n, then any 2 elements are merged in difference
//i.e. due to missing element the differnce expanded 
      if(missing.size() != 2) no;
      else (missing[0] + missing[1] == diff.back())? yes : no ;
      return;
    } 

    //now check for repeats, only 1 repeat can exist due to a missing element in permutation 
    // 3 4 1 2 5   
    // 3 7 8 10 15 
    //
    // delete 7 
    ...
    //
    // i.e. two 5s
    // one 5 can be broken down to 4 and 1 
    vll repeats;
    fori(0,n-2){
      if(diff[i] == diff[i+1]) repeats.pb(diff[i]);
    }
    ...
    //only 1 repaet found now check if its the sum of missing values so that it can be broken down in  a case 

    (missing[0] + missing[1] == repeats.front() ? yes : no);
