...
void test(){
  ...
  sort(p, p + n, greater<ll>()); // reverse sort

  ll sm[n+1] = {0};
  fori(1,n+1)//prefix sum of sorted prices starting with 0
    sm[i]+=sm[i-1] + p[i-1];
  //sm =  0 5 10 13 15 16--->test case example
  //here 0 denotes no item bought
  //5 denotes 1 item boght
  //next element denotes 2 items bought of  maximum price
  //and so on

    //strategy
    //customers buys x items with the maxed price
    //then y cheapest of them can be found out 
  ll x,y;
  fori(0,q)
  {
    cin >> x >> y;
    cout <<sm[x] - sm[x-y] << endl;
    //here sum of all x items - sum of items after removing y min items
    //actually gives us the sum of y items😀😀😀

    //below code gives tle hence try prefix sum
    /*ll sm = 0;
    forj(n-x, n-x+y)
      sm+=p[j];
    cout << sm << endl;*/
