...
void test() {
  ...
  // remove left and rightmost elements from array so as to compress from both
  // sides xxxxxxxx a a b c x v y zzzzzzz. Our target will be to remove the x
  // and z from corners.
  ll leftind = 0, rightind = n - 1;
  while (leftind < n && a[leftind] == a[0]) {
    leftind++;
  }
  while (rightind > -1 && a[rightind] == a[n - 1]) {
    rightind--;
  }

  ll ans = 0;
  if (a[0] == a[n - 1]) {
    ans = n - (leftind + n - 1 - rightind);
  } else {
    ans = n - max(leftind, (n - 1 - rightind));
  }
  ans = max(0ll, ans);
  cout << ans << endl;
}
