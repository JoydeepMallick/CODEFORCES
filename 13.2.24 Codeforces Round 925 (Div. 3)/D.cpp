...
void test(){
  ...
  moda.pb({a[0], 1});
  fori(1,n){
    ll last = moda[moda.size()-1].first;
    if(last == a[i]){
      moda[moda.size()-1].second++;
    }
    else{
      moda.pb({a[i], 1});
    }
  ...
  ll ans = 0, sz = moda.size();
  fori(0, sz-1){
    forj(i+1, sz){
      auto elei = moda[i], elej = moda[j];
      ll d = elei.first - elej.first;
      ll s = elei.first + elej.first;
      if(s%x == 0 && d%y==0){
        ans += elei.second * elej.second;
      }
    }
  }

  //now check for repeated eleements only 
  fori(0, sz){
    auto ele = moda[i];
