...
void test(){
  ...
  // 0 0 1 1 1 2 2 .. ...
  // valid  - 0 1 1,   1 2 2 ,   1 1 1   (3 different values violate the principle of 2 sides sum > third side )
  // when all elements equal ,  the always possible i.e.   freq(a[i]) C 3 (freq >= 3)
  // 
  // when 2 elements equal, the greater value must have frequency 2 (1 1 2  never valid but 1 2 2 is valid) 
  // now keep a sorted array and store frequency
  // ie. 0 -> 2 ,   1 --> 3,  2 -> 2, .....
  // if if element was 1, then freq[1] C 2 times  1 can be selected differently and all previous element (total count beofre) can be selected in previouscount C 1 ways.
  // hence final answer = freq[ele] C 2 * sumprevcount (hence sumprevcount must be  > 0 and freq >= 2 to be valid)

  map<int, int> freq;
  for(auto ele : a) freq[ele]++;

  ll ans = 0, total_no_of_elements_tillnow = 0;
  for(auto it : freq){
    ll fr = it.se;
    if(fr >= 3){
      ans += fr*(fr-1)*(fr-2)/6;  // N C 3
    }
    
    if(fr >= 2){ 
      ans += fr*(fr-1)/2 * total_no_of_elements_tillnow; 
    }
    total_no_of_elements_tillnow += fr;
  }
