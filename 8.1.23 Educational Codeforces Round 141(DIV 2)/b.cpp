...
void test(){
    int n;
    cin >> n;
/* it is always show n that for given conditon a matrix is always possbile such that for n n^2-1 distinct differences can be formed */
/*
 * n = 2
    1 4
    3 2
...
A zig zag pattern wherein 1st number starts from 1 and go on incresing by 1 but places at alternating positions
i.e. 1st 3rd 5th(considering the matrix to a 1d array)

Similarly 2nd element goes on decresing till last and placed in alternate positions i.e. 2nd 4th 6th .......
...
this kind of trend always follows
...
*/
    ...
    fori(0,n){
      if(i%2){//reverse printing
        forjdec(n*(i+1)-1, n*i-1, -1){
          cout << anew[j] << " ";
        }
        cout << endl;
      }
      else{
        //straight left to right printing
        forj(0, n){
          cout << anew[i*n + j] << " ";
        }
        cout << endl;
