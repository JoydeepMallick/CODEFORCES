...
void test(){
    ...
    if(distinct.size() == 1) no;
    else{
        //always possible if 2 distinct integers
/* remember that first 2 elements must be distinct to avoid ugliness then other numbers can be managed */
        //we can simply print array sorted in reverse order just avoiding the max element if freq more than 2 side by side in first part
        sort(a, a+n,greater<int>());
        int maxelefreq = distinct.rbegin()->second;
        if(maxelefreq > 1){
            a[1] = a[maxelefreq];//replace with 2nd element of sorted array with 2nd largest
            a[maxelefreq] = a[0];//its the max element
        }
        yes;
        fori(0,n) cout << a[i] << " ";
        cout << endl;
