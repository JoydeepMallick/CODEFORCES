...
struct node{
    //not just 2 but can be more children hence a problem
    int data;
...
void connect_node(int x, int y, struct node *nodes_list){
    //we need to connect inplace
    //vertex x has a child 
}
...
void test(){
    ...
    cin >> n;
    int val;
    struct node *nodes_list[n+1];
    fori(1,n+1){
        cin >> val;
        nodes_list[i] = newnode(val);
    }
    //vertex 1 is root
    int x,y;
    fori(0,n-1){
        cin >> x >> y;
        connect_node(x, y, nodes_list);
    }

    struct node *root = nodes_list[0];//given
}
