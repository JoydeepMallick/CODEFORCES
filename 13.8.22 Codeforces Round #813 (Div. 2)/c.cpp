...
void test(){
    ...
    int a[n];
    mii alive;
    int leastindexalive=-1;
    fori(0,n){
        cin >> a[i];//all positive integers
        alive[a[i]] = a[i];
    }
    int c = 0;
    fori(0,n){
        if(i == 0 || (i && alive[a[i]]>=alive[a[i-1]])) {
            if(leastindexalive==-1) leastindexalive = a[i];
            continue;
        }
        else {
            alive[a[i-1]] = 0;
            //when 0 0 3 0 3 2 no problem one operation manages last alive 3
            if(leastindexalive != a[i-1]) {
                alive[leastindexalive] = 0;
                c+=2;//when 0 0 1 0 3 2...where 1 is lastalive
            }
            else c+=1;
            leastindexalive = a[i];
        }
