...
void test(){
    ...
    auto it1=p[0].begin();
    fori(0,n){
        if(p[1].find(*it1) == p[1].end() && p[2].find(*it1) == p[2].end()){//all
                                                                           //unique
                                                                           //elements
            p1+=3;
            //p2+=3;
            ...
            //1st and 2nd player same word
            if(p[1].find(*it1) != p[1].end()){
                p[1].remove(*it1);
                p[0].remove(*it1);
                if( p[2].find(*it1) == p[2].end()){
                    p1++;
                    p2++;
                    //p3+=3;
                }
            }
            //1st and 3rd player word same
            else if(p[2].find(*it1) != p[2].end()){
                p[2].remove(*it1);
                p[0].remove(*it1);
                if(p[1].find(*it1) == p[1].end()){
                    p1++;
            ...
            //the last case when 3 words not same but 3 words not even unique,
            //so 2nd and 3rd word must be same
            else if(! (p[1].find(*it1) != p[1].end() && p[2].find(*it1) != p[2].end())){
                p[1].remove(*it1);
                p[2].remove(*it1);
                p2++;
                p3++;
