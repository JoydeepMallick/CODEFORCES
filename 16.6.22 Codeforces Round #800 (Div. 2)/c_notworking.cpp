...
void test(){
  ...
  int n;
  cin >> n;
  int a[n], ps[n+1] = {0};//prefix sum of array a
  fori(0,n)
  {
    cin >> a[i];
    ps[i+1] = ps[i] + a[i];
  ...
  /*
   since pointer starts from first element and add 1 as it moves right to
  an array of 0s and adds -1 as it moves to left of array and finally it MUST REACH FIRST ELEMENT, we can surely tell that our formed array has sum equal to 0.
  hence if sum of array a is not 0 hence NOt possible.
  */
  if (ps[n] != 0){
    cout << "NO" << endl;
  }
  else{
    bool ok = 1; //assumed possible
    fori(1,n+1){
      if(ps[i] < 0) ok = 0;//if any negative sum is present in ps not possible
    }
    if(ok){
      //if no negative was found in prefix sum of elements of a
      //then check if sum is 0 between two non zero sum.
      //Its not possible , 0 can only exist from end in a continous fashion
      //e.g. ps = 0 2 1 2 0 0 0 possible
      //          0 2 1 2 0 1 0 not possible
      bool visited_0 = 0;
      fori(1,n+1){
        if(ps[i] == 0){ visited_0 = 1;}
        else if(visited_0) {ok = 0;}
      }
    }
