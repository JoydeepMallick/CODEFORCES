...
void test(){
  ...
  when we encounter 2 same adjacent characters i.e a[i] == a[i+1] we cant tell that as paranoid hence we need to skip that same pair.

  whenever a[i] not equal a[i+1] we can reduce it to either 0 and 1.
  and make it paranoid. We also notice that all the substrings possible from index 0 till index x+1 are valid paranoids and each individualdigit is obvoiusly paranoid.

  eg. 1   0   0   1   0   1
ind   0   1   2   3   4   5
         +1      +3  +4  +5   + no.of elements
  */
  ull c = n;
  fori(1,n)
  {
