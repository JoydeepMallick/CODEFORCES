...
void test(){
  int n;
  cin >> n;
  int a[n+1]; // following 1 based indexing i.e a[0] = 0
  fori(1,n+1){ 
    cin >> a[i];
  }
  while(n > 0 && a[n] == 0) --n;
  //finding the index at which continuous streak of ending zeros is broken 
  //i.e 1 0 -1 0 0 0 0
  //         ^ here n = 3

  if (n == 0) {
    //finding out if all elements are 0
    cout << "YES" << endl;
  }
  else if(a[n] > 0){
    //it can be observed that in process of going back to intial position we will always
    //encounter that a negative occurs before a continous streak of 0s from end.
    //i.e .....-2 0 0 0...0s only.
    //hence if nth element is > 0 its not possible to create one
    cout << "NO" << endl;
  }
  else{
    /* 
     * Observation :-
     *
     * first element is always positive(it always can increase only from 0)
     * a[n] can never be 0(provided we reach else part) and will always be negative(can always decrease from 0)
     *
     */
    int sum = -a[n], i = --n;
