...
void test(){
  int n;
  cin >> n;
  int x,y,max_x= INT_MIN,max_y= INT_MIN,min_x= INT_MAX,min_y = INT_MAX;
  fori(0,n){
    cin >> x >> y;
    if(x > max_x) max_x = x;
    if(y > max_y) max_y = y; 
  ...
  y = (max_y - min_y);
  bool ori_btwn = false;
  if(max_x >= 0 && max_y >= 0 && min_y <= 0 && min_x <= 0) ori_btwn = true;
  //when x and y both  have some value then result path is equivalent to
  //rectangle though we start and end at orgin
  //but when either one is present then only same path traversed twice
  if(ori_btwn) cout << 2*x + 2*y << endl;
  else{
    max_x = (abs(max_x) > abs(min_x) ? abs(max_x) : abs(min_x));
    max_y = (abs(max_y) > abs(min_y) ? abs(max_y) : abs(min_y));
    cout << 2 * max_x + 2 * max_y << endl;
  }
}
