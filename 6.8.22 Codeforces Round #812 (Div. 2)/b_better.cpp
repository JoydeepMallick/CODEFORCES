...
void test(){
  int n,ele,c = 0;
  int last = -1; //since all elements are sure to be greatrer than 0 as per constraints
  cin >> n;
  bool ok = 1;
  fori(0,n){
    cin >> ele;
    //same logic but better code
    if(ele >= last && c == 0) last = ele;//non decreasing check
    else if(ele < last && c == 0){last = ele; c = 1;}//now only check for non increasing
    else if(ele <= last && c == 1) last = ele;
    else ok = 0;//now you hit an ele which breaks the non increasing order, the array is not optimal for sure
  }
  if(ok) yes;
  else no;
}
