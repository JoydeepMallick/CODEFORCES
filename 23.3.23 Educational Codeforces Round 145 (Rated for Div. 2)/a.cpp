...
void test(){
    ...
    for(char c:s) distinct[c]++;
    dbg(distinct);
    int ans;
    if(distinct.size() == 1) ans = -1;
    else if(distinct.size() >=3) ans = 4;
    else{
      // a a b b    or a a a b  
      // 4              6  
     dbg((distinct.begin())->second); 
      if(distinct.begin()->second == 1 or distinct.rbegin()->second==1) ans = 6;
      else ans = 4;
    }
    cout << ans << endl;
