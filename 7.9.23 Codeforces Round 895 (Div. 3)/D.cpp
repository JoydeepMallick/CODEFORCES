...
/* The number of red indices is ⌊nx⌋ */

/* , and the number of blue indices is calculated similarly. */

/* To calculate the number of purple indices, we need to notice that the condition "index is divisible by both x */
/* and y" is equivalent to "index is divisible by lcm(x,y)", where lcm denotes the least common multiple. Therefore, the number of purple indices is ⌊nlcm(x,y)⌋ */

/* . */

/* Let R */
/* be the number of "red but not purple" indices, and let B be the number of "blue but not purple" indices. Then it is not difficult to see that the maximum score is (n+(n−1)+…+(n−(R−1))−(1+2+…+B) */

/* . */

/* To quickly find this sum, we can use the formula for the sum of an arithmetic progression: l+(l+1)+…+r=(l+r)⋅(r−l+1)2 */


    ll R = floorl(n/x), B = floorl(n/y), P = floorl(n/lcm(x,y));
    ll Ronly = R - P, Bonly = B - P;

    ll sumR = (n- Ronly+1 + n)*(n - n + Ronly - 1 + 1)/2;
    ll sumB = Bonly*(Bonly+1)/2;
    dbg(Ronly, Bonly, sumR, sumB);
    cout << sumR - sumB << endl;
}
