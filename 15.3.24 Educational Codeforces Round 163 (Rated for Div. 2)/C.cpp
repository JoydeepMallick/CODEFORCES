...
bool f(vector<vector<char>> &dir, ll curi, ll curj){
   
      if(curi == 2 and curj == dir[1].size()-1){
          return true;
      }
    if(curi == 1){
        if(dir[2][curj] == '>'){
            return f(dir, 2, curj+1);
        }
        else if(dir[1][curj+1] == '>'){
            return f(dir, 1, curj+2);
        }
        else return false;
    }
    if(curi == 2){
        if(dir[1][curj] == '>'){
            return f(dir, 1, curj+1);
        }
        else if (dir[2][curj+1] == '>'){
            return f(dir, 2, curj+2);
        }
        else return false;
    }
...
void test(){
  ll n;
  cin >> n;
  vector<vector<char>> dir(3, vector<char>(n+1));
  fori(1,3){
      forj(1,n+1) cin >> dir[i][j];
  }
  dbg(dir);

  ll curi = 1, curj = 1;
  ll lasti = 1, lastj = 1;
    if(f(dir, curi, curj)) yes;
    else no;
}
