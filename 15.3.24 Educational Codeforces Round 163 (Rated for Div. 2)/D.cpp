...
void test(){
    ...
    int assumedhalfsize = s.size()/2; // max length of firsthalf of tandem string 
    while(true){
        int halfsize = 0;
        fori(0, s.size()-assumedhalfsize){
            if(s[i] == s[i+assumedhalfsize] || s[i] == '?' || s[i+assumedhalfsize] == '?') halfsize++;
            else halfsize = 0; // reset first half possible size and see from next index again

            if(halfsize >= assumedhalfsize){
                cout << 2 * assumedhalfsize << endl;
                return;
            }
        }
