...
Given any integer x
, we can see that its lowest significant bit is a divisor of x. If x has more than one bit, we can repeatedly subtract the value corresponding to the lowest significant bit of x. When x finally has only one bit, finish the first step. In this step, we have only used each significant bit of x
...
We can find a way to reduce 2L
to 1 by using each bit exactly once. Formally, if k≥0, then 2k+1−2k=2k, and 2k is a divisor of 2k+1. Thus, by subtracting 2L−1,2L−2,…,1 in order, we reach 1 from 2L by using each bit from the 0-th bit to the (L−1)

-st bit exactly once.

As a result, we can reduce x
to 1 by using each power of 2 at most twice (once from the first step, once from the second step). Since we used each bit at most twice, the time complexity for solving one test case is O(logx)

.

Due to the lenient constraints, some solutions with O(x−−√)
time complexity should pass as well (as long as they fit into the 1000 operations limit).
 */
        int powerof2;
        a.pb(n); 
        for(int i = 0; ; i++){
