...
void bfs(vector<vector<int>> &lake, vector<vector<int>> &vis, int row, int col, ll &depth){
    ...
    fori(0,4){
        ...
        if(
                0<= nxtrow && nxtrow < lake.size() 
                && 0 <= nxtcol && nxtcol < lake[0].size()
                && !vis[nxtrow][nxtcol] && lake[nxtrow][nxtcol] > 0) bfs(lake, vis, nxtrow, nxtcol, depth);
    }
}
...
void test(){
    int n,m;
    cin >> n >> m;
    vector<vector<int>> lake(n, vector<int>(m));//bigest lake 
    ll ans = 0;
    fori(0,n){
        forj(0,m) cin >> lake[i][j];
    }
    
    vector<vector<int>> vis(n, vector<int>(m,0));
    //traverse all cells so that no componets are missed
/* dbg(lake, vis); */
    fori(0,n){
        forj(0,m){
            if(!vis[i][j] && lake[i][j]>0) {
                ll depth = 0;
                bfs(lake, vis, i, j, depth);
                ans = max(depth , ans);
            }
        }
    }
