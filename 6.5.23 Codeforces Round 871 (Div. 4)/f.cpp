...
void test(){
    int n,m;
    cin >> n >> m;
    int degree[n+1] = {0};// store degree of each vertex 
    fori(1,m+1){
        int u,v;
        cin >> u >> v;
        degree[u]++;
    ...
    //all  vertices with degree one are leaves and hence their cnt/x gives y  
    //x can have a case when degree of central vertex = x then we need to check if we have 3 unique degree values of vertices and so will the answer be 
    map<int, int> cntdegree; // stores freq of different degree occurences
    fori(1, n+1){// for all vertices
        cntdegree[degree[i]]++;
    }
    
    dbg(cntdegree);
    //when size 3 then if freq was 1 its central vertex, and when degree 1 its leaf other one is the count for x 
    //when size 2 then degree 1 is leaf vertex and other freq value - 1 is the value of x
    //y can be obtained by total cnt of degree 1 / x 
    vector<int> freq_cntdegree;
    for(auto it : cntdegree) freq_cntdegree.pb(it.second);
    sort(all(freq_cntdegree));

    dbg(freq_cntdegree);
    if(freq_cntdegree.size()==3){
        //1 means central vertex in freq_cntdegree 
        cout << freq_cntdegree[1] << " " << freq_cntdegree[2]/freq_cntdegree[1] << endl; 
    }
    else{
        cout << freq_cntdegree[0]-1 << " " << freq_cntdegree[1]/(freq_cntdegree[0]-1) << endl;
    }
}
