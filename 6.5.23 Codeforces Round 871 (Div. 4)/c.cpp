...
void test(){
    ...
    types["00"] = types["01"] = types["10"] = types["11"] = 1e6;//set all to extremely large values 
    fori(0,n){
        cin >> m;
        cin >> s;
        types[s] = min(types[s], m);    
    }
    if(min(types["11"], types["01"] + types["10"]) < 1e6) cout << min(types["11"], types["01"] + types["10"]) << endl;
    else cout << "-1\n";
