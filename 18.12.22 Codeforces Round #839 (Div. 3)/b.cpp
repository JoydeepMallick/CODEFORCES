...
void test(){
  /*
    ...
    cin >> mat[0] >> mat[1];
    cin >> mat[2] >> mat[3];
    //2 numbers can be same but grater than 1 and smaller than other
    // a and b any one max or min
    // else c and d any one max or min
    if(mat[0]==mat[1] || mat[2]==mat[3] || mat[0]==mat[2] || mat[1]==mat[3]){
      no;
      return;
    }
    int &mn = *min_element(mat, mat+4);
    int &mx = *max_element(mat, mat+4);

    if((mn==mat[0] && mx==mat[3])||(mn==mat[3] && mx==mat[0]) || (mn==mat[1] && mx==mat[2])||(mn==mat[2] && mx==mat[1])) yes;
    else no;
    */
        vector<int> a(4);
        for(int i = 0; i < 4; i++)
        ...
        //returns address of element and here we receive index as subtracted
        //from start address of vector and
        //* returns value
        int minpos = min_element(a.begin(), a.end()) - a.begin();
        if(maxpos + minpos == 3)//1+2 or 0+3
            puts("YES");
        else
            puts("NO");
}
