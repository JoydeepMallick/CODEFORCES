...
void test(){
    ...
    forj(1,n+1){
        cin >> a[i];
        if(i == 1) breakpoints.push_back(i);
        else if(a[i-1] != a[i]) breakpoints.push_back(i);
    }
    forj(0,m){
        cin >> i >> x;
        if(((i-1) && a[i-1] == x)||((i+1)<= n && a[i+1] == x)){
            if(breakpoints.find(i) != breakpoints.end()){
                int toput = ((i-1) && a[i-1] == x)?i-1:i+1;
                breakpoints.push_back(toput);
                breakpoints.remove(i);
            }
            else{
                breakpoints.push_back(i);
                if(i+1 <= n) breakpoints.push_back(i+1);
            }
        }
    }
}
