...
void test(){
    ...
    ll mintillnow = a[0];
    minsumpref[0] = mintillnow;
    fori(1,n){
        mintillnow = min(mintillnow, a[i]);
        minsumpref[i] = minsumpref[i-1] + mintillnow;
    }

    //sliding window by merging adjacent
    ll minans = minsumpref[n-1];//no operations done 

    ll mintillbefore = INT_MAX;
    fori(0, n-1){
        if(i > 0) mintillbefore = min(mintillbefore, a[i-1]);
        ll newai = a[i] + a[i+1];
        ll ans = (i > 0) ? minsumpref[i-1] : 0;
        ans += min(mintillbefore, newai);//rest all from i+1 set to 0 
