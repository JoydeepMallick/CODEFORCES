...
void test(){
  ...
  vector<ull> a(n); 
  read(a);
  unordered_map<bitset<31>, ll> cntele;
  fori(0,n){
    cntele[bitset<31>(a[i])]++;
  }

/* ull all1 = (1ull << 31) - 1ull; */
  ll paircnt = 0; // max group size is always 2
  fori(0,n){
    bitset<31> ele(a[i]);
    if(cntele[ele] == 0) continue;

/* ull opponum = all1 ^ a[i]; */
    bitset<31> opponum = oppo(ele); 
    if(cntele[opponum] > 0) cntele[opponum]--; // a pair made if possible
