...
void test(){
  int n,m;
  cin >> n >> m;
  int a[n],fromleft[n],fromright[n];
  fori(0,n){ 
    cin >> a[i];
    if(i != 0){
      fromleft[i] = fromleft[i-1] + (a[i] - a[i-1])<0?(a[i-1]-a[i]) : 0; 
      fromright[i] = fromright[i-1] + (a[i] - a[i-1])<0? 0 : (a[i]-a[i-1]);
    }
  }
  ...
  fori(0,m){
    ...
    cin >> s >> t;
    if(s>t){
      sm = fromright[s-1] - fromright[t-1];
    }
    else {
      sm = fromleft[t-1] - fromleft[s-1];
    }
    cout << sm << endl;
  }
}
