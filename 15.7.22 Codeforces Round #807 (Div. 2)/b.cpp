...
void test(){
  ...
  fori(0,n){
    ...
    if(a[i]!=0 && i!=(n-1)){
      if(start==-1) start = i;
      s += a[i];//sum of all non zero lement till 2nd last
    }
  }
  //clean all and make last room dirtiest
  //....3 0 5 9 0....0 7 6 3. note when we hit last 0 after that all non zero
  //elements before last elements require individual steps.
   fori(start+1,n-1){
    if(a[i] == 0) s++;
   }
   if(start == -1) s = 0;//if never a non zero elemtn encountered till 2nd last element
  cout << s << endl;
}
